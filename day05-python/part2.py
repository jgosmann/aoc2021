...
class DangerousPointCounter:
    ...
    def count_line(self, line: Line):
        if line.start.x == line.end.x:
            ...
            self._count_diagonal_line(line.start, line.end)
        else:
            raise ValueError("line must be horizontal, vertical, or 45-degree diagonal")
    ...
    def _count_diagonal_line(self, start: Point, end: Point):
        if start.x > end.x:
            start, end = end, start
...
def test_number_of_dangerous_points():
    ...
    assert counter.num_of_dangerous_points == 5
    counter.count_line(Line(start=Point(7, 5), end=Point(4, 2)))
    assert counter.num_of_dangerous_points == 7
    counter.count_line(Line(start=Point(7, 2), end=Point(4, 5)))
    assert counter.num_of_dangerous_points == 8


def test_count_dangerous_points():
    with open("test.input", "r") as f:
        assert count_dangerous_points(f) == 12
